...
앱을 실행할 때 현재 실행 파일을 사용자별 Windows 자동 시작 항목(`HKCU\Software\Microsoft\Windows\CurrentVersion\Run\ScheduleWidget`)에 등록합니다. 레지스트리 키가 없으면 생성하고, 등록한 값도 다시 읽어 성공 여부를 확인합니다. 권한이나 레지스트리 오류로 등록하지 못해도 앱은 계속 실행되며 시작 후 경고를 표시합니다.
